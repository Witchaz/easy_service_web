...
export default function MachineList() {
  ...
  const handleEdit = (index: number) => {
    navigate("/machineEdit", {
      state: { machines: formData.details, index, formData },
    });
  };
      ...
      <div className="flex flex-col items-center min-h-screen bg-gray-100">
        ...
        <div className="bg-white p-6 rounded-lg shadow-md w-full max-w-2xl h-[500px] overflow-y-auto">
          {formData.details.map((machine: Machine, index) => (
            <div
              key={index}
              className="bg-gray-50 p-4 mb-4 rounded-md flex justify-between items-center"
            >
              <div>
                <h3 className="font-semibold mb-1">
                  รายละเอียดเครื่องซ่อมลำดับที่ {index + 1}
                </h3>
        ...
        <div className="flex justify-between mt-8 space-x-16">
          ...
          <button
            className="bg-lime-500 text-white font-semibold px-4 py-2 rounded-lg hover:bg-lime-600"
            onClick={handleSave}
          >
            Save
          </button>
