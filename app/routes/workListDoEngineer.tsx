...
export default function workListSEngineer() {
  ...
  useEffect(() => {
    const fetchWorks = async () => {
      const url = `https://easy-service.prakasitj.com/works/getWorksListByStatus/6`;
      const options = { method: "GET" };
  ...
  const handleSelect = (workId: number) => {
    navigate("/engineerDoWork ", { state: { workId } });
  };
      ...
      <div className="flex flex-col items-center min-h-screen bg-gray-100">
        ...
        <div className="w-full max-w-4xl h-[500px] overflow-y-auto space-y-6">
          {works.length > 0 ? (
            ...
            <p className="text-center text-lg font-semibold text-gray-600">ยังไม่มีงานที่ต้องไปติดตั้ง</p>
          )}
        </div>
