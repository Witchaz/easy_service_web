...
export default function WorkConfirm() {
  ...
  const handleSave = async () => {
    const warrantyDetails = formData.details.filter(
      (detail: { warranty: boolean }) => detail.warranty === true
    );
    const nonWarrantyDetails = formData.details.filter(
      (detail: { warranty: boolean }) => detail.warranty === false
    );

    if (warrantyDetails.length > 0 || nonWarrantyDetails.length > 0) {
      try {
        const urlCustomerID = `https://easy-service.prakasitj.com/customers/getIDbyName/${encodeURIComponent(
          formData.customerName
        )}`;
        const optionsCustomerID = { method: "GET" };
        const responseCustomerID = await fetch(
          urlCustomerID,
          optionsCustomerID
        );
        ...
        if (warrantyDetails.length > 0) {
          const urlCreateWork =
            "https://easy-service.prakasitj.com/works/createNewWork";
          ...
          const urlWorkID =
            "https://easy-service.prakasitj.com/works/getLastWork";
          ...
          for (const detail of warrantyDetails) {
            const urlCreateRequests =
              "https://easy-service.prakasitj.com/Requests/insertRequest";
      ...
      <div className="flex flex-col items-center min-h-screen bg-gray-100">
        ...
        <div className="bg-white p-6 rounded-lg shadow-lg w-[90%] max-w-4xl max-h-[550px] overflow-y-auto mb-6">
          <div className="border-b border-gray-300 py-4">
            <p>
              <strong>ชื่อลูกค้า:</strong> {formData.customerName}
            </p>
            <p>
              <strong>สถานที่ซ่อม:</strong> {formData.address}
            </p>

            {formData.details.length > 0 ? (
              formData.details.slice(0, 4).map(
                (
                  detail: {
                  ...
                  <p key={detail.id}>
                    <strong>รายละเอียดเครื่องซ่อมลำดับที่ {index + 1} :</strong>{" "}
                    Model: {detail.model}, Serial Number: {detail.serialNumber}
                  </p>
                )
              )
            ) : (
            ...
            <div className="flex space-x-4 mt-4">
              <button
                className="bg-lime-400 hover:bg-lime-500 text-white font-semibold px-4 py-2 rounded-lg"
