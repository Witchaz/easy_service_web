...
export default function adWorkList() {
  ...
  const fetchAdditionalCost = async (work_id: number): Promise<number> => {
    ...
    try {
      ...
      const data = await response.json();
      return data.reduce(
        (total: number, cost: { cost: number }) => total + cost.cost,
        0
      );
    } catch (error) {
  ...
  const handleNewButtonAction = async (workId: number) => {
    ...
    if (!work.user_id) {
      alert(
        "This work has no assigned engineer. Please assign an engineer first."
      );
      return;
    }
    ...
    if (!work.machines || work.machines.length === 0) {
      alert(
        "This work has no machines. Please ensure there is at least one machine."
      );
      return;
    }
            ...
            works.map((work) => (
              ...
              >
                <div>
                  ...
                  <p>
                    <strong>สถานที่ซ่อม:</strong> {work.address},{" "}
                    {work.province}
                  </p>
                  {work.machines &&
                  ...
                  <p>
                    <strong>ช่างผู้รับผิดชอบ:</strong> {work.userName || "-"}
                  </p>
                  <p>
                  ...
                  <p>
                    ...
                    {work.additionalCost?.toFixed(2) || "0"}
                  </p>
                  <p>
