...
export default function EngineerAddSpareParts() {
  const { SparePartsList, total, q, page } = useLoaderData<LoaderData>();
  const submit = useSubmit();
  const navigate = useNavigate();
  const location = useLocation();
  const { workId, machineId } = location.state || {}; // Retrieve workId and machineId from state
      ...
      <div className="min-h-screen p-8 bg-gray-50">
        ...
        <div className="bg-white p-4 shadow-md rounded-lg">
          ...
          <div className="flex justify-center space-x-24 mt-8">
            <button className="bg-black text-white py-2 px-6 rounded-lg hover:bg-gray-600" onClick={handleBack}>Back</button> 
