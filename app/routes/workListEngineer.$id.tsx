...
export const loader = async ({ params }: LoaderFunctionArgs) => {
  ...
  if (!response.ok) {
    throw new Response("Failed to fetch work details", { status: response.status });
  }
          ...
          {works.map((work) => (
            <div key={work.id} className="bg-gray-50 p-6 rounded-lg shadow-md flex justify-between items-start">
              <div>
                ...
                <p><strong>ชื่อลูกค้า:</strong> {work.customer_id}</p>
                ...
                <p><strong>ช่างผู้รับผิดชอบ:</strong> {work.customer_id || "-"}</p>
